...
| 统计工具 | 现阶段状态 | 启用方式 | 说明 |
| --- | --- | --- | --- |
| **Microsoft Clarity** | ✅ **正常接入启用**（行为热力图/会话回放兜底） | 填入 Project ID 即生效 | 代码已就绪，非备案阻塞，仅缺凭证 |
| **不蒜子 Busuanzi** | ✅ **已启用**（前台计数器） | 无需配置，默认加载 | 页脚 Chip 已渲染，脚本已自动加载 |
| **Cloudflare Web Analytics** | ✅ **已启用**（ICP 备案鄂ICP备2026045053 已通过，token 已填入） | 构建上线即生效，无需再改 | `index.html` 中 `CF_TOKEN` 已替换为 `0d21849e…`，beacon.min.js 延迟加载，5 分钟后可在 CF 后台看实时 UV/PV |

> **现阶段状态更新（备案已通过）**：Cloudflare Web Analytics 作为核心流量大盘已正式启用，结合 Clarity + 不蒜子构成三层统计体系，完整覆盖日 UV/PV、访问来源、设备、地域、页面热度、用户点击热力图、会话回放、前台访问量展示，支撑引流复盘与广告优化。
