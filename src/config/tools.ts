...
export const TOOLS_CONFIG: Record<string, ToolConfig> = {
  ...
  'image-removebg': {
    ...
    name: 'AI 抠图工具',
    shortDesc: '基于 RMBG-2.0 (BiRefNet) 浏览器本地高精度 AI 一键抠图，WebGPU 加速 + IndexedDB 模型缓存，默认白底、支持透明/自定义纯色，上传即抠、背景切换实时预览，单张 PNG / 批量 ZIP 自动智能压缩',
    category: 'image',
    icon: '🪄',
    title: 'AI 抠图工具 去背景变透明 - MendFile 全能办公工具（RMBG-2.0 · 纯前端本地推理 · 不上传图片）',
    description: 'MendFile 免费 AI 抠图工具，基于当前浏览器端开源商用顶配 RMBG-2.0 (BiRefNet) 高精度抠图模型 + ONNX Runtime Web 纯前端本地推理，WebGPU/WebGL/WASM 自动降级；模型永久缓存至浏览器 IndexedDB，首次下载约 176MB，之后秒级启动。人像、商品、证件照、发丝细节效果对标主流商用在线抠图；上传即抠图、背景切换实时预览，默认白底可切透明或自定义纯色，智能压缩限制最长边 ≤ 2400px；单张直接下载 PNG、批量打包 ZIP；所有图片数据 100% 不离开用户设备，无后端、无登录、无次数限制。',
    keywords: ['在线AI抠图', '去背景变透明', '高精度AI抠图', '证件照换底', '人像抠图', '商品抠图', '发丝抠图', '透明PNG生成', '批量抠图', '前端本地抠图', '无后端抠图'],
    features: [
      '【核心引擎】RMBG-2.0 (BiRefNet) 高精度抠图模型：ONNX Runtime Web 推理，WebGPU → WebGL → WASM 自动降级',
      '【模型缓存】模型 ~176MB 永久缓存在浏览器 IndexedDB，首次下载一次，之后秒级可用，无需重复加载',
      '【效果】像素级精准抠图：发丝、人像、证件照、常规商品边缘细节，边缘过渡自然无毛边/灰边/残留背景',
      '【上传即抠】用户上传图片瞬间自动触发抠图，零操作等待；抠图完成后自动生成成品预览卡',
      '【实时预览】白底 / 透明底 / 自定义纯色 三模式切换，预览图立即实时重绘，全程所见即所得',
      '【设置持久化】背景选择、自定义颜色、压缩偏好自动保存到本地存储，下次进入工具自动复用',
      '【输出优化】智能轻量化压缩：最长边 ≤ 2400px + 按背景择优 PNG/JPG/WebP；单张直接下载 PNG，批量自动打 ZIP',
      '【自动降级】AI 模型加载 / 推理异常时，自动降级为 Flood Fill 颜色容差法兜底，保证工具 100% 可用',
      '【隐私安全】全程纯前端本地推理，图片数据不离设备，无后端、无登录、无次数限制、无文件上传',
    ],
    boundaries: [
      '✅ 擅长：日常人像 / 电商商品 / 证件照 / 毛发发丝 / 服装布料等常见主体抠图（效果对标主流水准）',
      '⚠️ 仍有难度：多层重叠遮挡、超低分辨率 / 严重模糊图像、精细半透明物体（薄纱 / 玻璃 / 烟雾）等场景可能存在边界误差',
      '❌ 正式用途提醒：身份证、护照、签证、考试报名等合规证件照，请以专业照相馆或官方指定工具出图为准',
      'ℹ️ 浏览器能力：推理依赖 WebGPU / WebGL / WASM，老旧浏览器会自动降级 WASM，速度会略慢但仍可正常使用',
