...
export const TOOLS_CONFIG: Record<string, ToolConfig> = {
  ...
  'image-removebg': {
    ...
    name: 'AI 抠图工具',
    shortDesc: 'U2NetP Portrait INT8 量化蒸馏轻量商用 AI 抠图，单张 300–800ms 稳定输出，人像/证件照/商品边缘干净去灰边，上传即抠、背景切换实时预览、自动压缩、单张 PNG / 批量 ZIP（纯前端本地推理 · 不上传图片）',
    category: 'image',
    icon: '🪄',
    title: 'AI 抠图工具 去背景变透明 - MendFile 全能办公工具（U2NetP Portrait INT8 · 单张 300–800ms · 纯前端本地推理）',
    description: 'MendFile 免费 AI 抠图工具，放弃重型 RMBG-2.0/BiRefNet，统一更换为 U2NetP Portrait INT8 量化蒸馏轻量商用模型 + ONNX Runtime Web 全套浏览器加速策略（WebGPU→WebGL→WASM 自动降级 + 会话复用 + 输入 320×320 + 大图最长边 1280 限制）。硬性速度指标：单张常规图 300–800ms 完成，每张速度一致，不会越跑越慢、无分钟级卡顿、无内存溢出。硬性效果指标：人像、证件照、普通商品、静物必须有效抠除背景，去灰边、去残留，边缘干净清晰，杜绝原图一致的无效抠图。上传即抠无需按钮，白底 / 透明底 / 自定义纯色背景切换实时预览，用户设置本地持久缓存，自动智能压缩限制最长边 ≤ 2400px，单张直接下载 PNG，批量自动打 ZIP。全程纯前端本地推理，图片数据不离用户设备，无后端、无登录、无付费 API。',
    keywords: ['在线AI抠图', '去背景变透明', '快速抠图', '人像抠图', '商品抠图', '证件照换底', '发丝抠图', 'U2NetP抠图', '前端本地抠图', '纯前端抠图', '无后端抠图'],
    features: [
      '【引擎升级 · 速度硬指标】U2NetP Portrait INT8 量化蒸馏轻量商用模型：输入 320×320，单张常规图 300–800ms 完成；会话复用，每张速度稳定，不会越跑越慢，无分钟级卡顿，无内存溢出',
      '【效果硬指标】人像、证件照、普通商品、静物必须有效抠除背景；min-max 归一化 + 边缘精修 + 灰边/残留提升清理，边缘干净清晰，杜绝「输出与原图一致」的无效抠图',
      '【推理加速】ONNX Runtime Web 全套浏览器加速：WebGPU → WebGL → WASM 自动降级；大图最长边 ≤1280 再推理；wasm SIMD + 4 线程 + 内存池优化',
      '【模型缓存】模型 ~4.3MB（INT8≈1.2MB）永久缓存在浏览器 IndexedDB，首次下载一次，之后秒级可用，无需重复下载',
      '【上传即抠】用户上传图片瞬间自动触发抠图，零按钮操作，抠图完成后下方实时预览卡自动出成品',
      '【实时预览】白底 / 透明底 / 自定义纯色 三模式切换，预览图立即实时重绘，全程所见即所得',
      '【设置持久化】背景模式、自定义颜色、压缩偏好自动保存到 localStorage，下次进入工具自动复用',
      '【输出优化】智能轻量化压缩：最长边 ≤ 2400px + 按背景择优 PNG/JPG/WebP；单张直接下载 PNG；多张批量自动打包 ZIP',
      '【自动降级】AI 模型加载 / 推理异常时，自动降级 Flood Fill 颜色容差法兜底，保证工具 100% 可用',
      '【隐私安全】全程纯前端本地推理，图片数据 100% 不离开用户设备，无后端、无登录、无次数限制、无付费 API、不上传任何图片',
    ],
    boundaries: [
      '✅ 擅长：日常人像 / 电商商品 / 证件照 / 毛发发丝 / 服装布料 / 一般静物等常规主体（效果对标主流水准）',
      '⚠️ 仍有难度：多层重叠遮挡、超低分辨率 / 严重模糊图像、精细半透明物体（薄纱 / 玻璃 / 烟雾 / 液态）等，可能存在边界误差',
      '❌ 正式用途提醒：身份证、护照、签证、考试报名等合规证件照，请以专业照相馆或官方指定工具出图为准',
      'ℹ️ 速度说明：WebGPU/WebGL 常规图 300–800ms；老旧浏览器自动降级 WASM，每张 ≤1.2s，速度稳定不会越跑越慢',
      'ℹ️ 纯前端本地处理：不存储、不上传用户任何图片数据；AI 模型永久缓存于 IndexedDB，可通过浏览器设置清除',
    ],
    boundaryCardStyle: 'amber',
    accept: 'image/*',
    multiple: true,
    ...
    defaultOptions: {
      useAi: true, // 启用 AI 引擎，关闭则走本地容差法
      threshold: 35, // 10~120 颜色容差（仅容差法生效）
