...
export const TOOLS_CONFIG: Record<string, ToolConfig> = {
  ...
  'image-removebg': {
    ...
    name: '抠图工具',
    shortDesc: '纯 Canvas 像素抠图，零模型零卡死，上传即抠 · 背景实时切换 · 自动压缩 · 单张 PNG / 批量 ZIP，纯前端本地处理、不上传、无 API、无付费',
    category: 'image',
    icon: '🪄',
    title: '抠图工具 去背景变透明 - MendFile 全能办公工具（纯 Canvas 像素算法 · 页面绝不卡死 · 纯前端本地处理）',
    description: 'MendFile 免费抠图工具，彻底放弃 RMBG / U2Net / BiRefNet 等所有前端 AI 模型、ONNX、WebGPU、WASM 推理方案，统一重写为纯 Canvas 像素算法：四角+四边+2%边带采样背景颜色，k-means 聚类多代表色，颜色距离+软过渡带宽生成 alpha，再用 Flood Fill 收紧背景，最后做边缘精修、灰边清理、可选羽化。速度硬保障：大图自动缩到最长边 1600 内部处理，像素大循环每 32 行分片 yield 一次事件循环，页面绝不卡死、无分钟级卡顿、不占用超高资源；每张速度稳定，不会越跑越慢。效果硬保障：所有像素必改 alpha，杜绝原图一致的无效抠图，满足日常办公（白底证件照、电商白底图、人像纯色背景、普通商品静物）。上传即自动抠无需按钮，白底 / 透明底 / 自定义纯色背景实时切换预览，用户设置本地持久缓存，自动智能压缩最长边 ≤ 2400px、按背景择优 PNG/JPG/WebP，单张直接下载、批量自动 ZIP。全程纯前端本地处理，图片数据不离用户设备，无后端、无登录、无付费 API。',
    keywords: ['在线抠图', '去背景变透明', '快速抠图', '人像抠图', '商品抠图', '证件照换底', '白底图', '纯前端抠图', '本地抠图', '无后端抠图', '不卡死抠图'],
    features: [
      '【极致稳定 · 杜绝卡死】纯 Canvas 像素算法：零 AI / 零 ONNX / 零 WebGPU / 零 WASM 依赖，不下载任何模型，零分钟级卡顿风险；像素大循环每 32 行分片 yield 事件循环，大图自动缩到最长边 1600 内部处理，每张速度稳定、不会越跑越慢',
      '【效果稳定可用】四角+四边中点+2%边带像素采样，k-means(k≤3) 聚类得到背景多代表色；颜色欧氏距离+S 曲线软过渡 + Flood Fill 收紧背景 + 边缘精修 + 灰边清理，所有像素必改 alpha，杜绝「输出与原图一致」的无效抠图',
      '【上传即抠】用户上传图片瞬间自动触发抠图，零按钮操作，抠图完成后下方预览卡自动出成品',
      '【实时预览】白底 / 透明底 / 自定义纯色 三模式切换，预览图立即实时重绘，全程所见即所得',
      '【设置持久化】背景模式、自定义颜色、容差阈值、羽化、压缩偏好自动保存到 localStorage，下次进入工具自动复用',
      '【输出优化】智能轻量化压缩：最长边 ≤ 2400px + 按背景择优 PNG/JPG/WebP；单张直接下载 PNG（WebP/JPG 可用时自动更小）；多张批量自动打包 ZIP',
      '【极端容错】即使异常也至少输出原图压缩结果，保证工具 100% 可用；所有错误都不会让页面冻结',
      '【隐私安全】全程纯前端 Canvas 像素处理，图片数据 100% 不离开用户设备，无后端、无登录、无次数限制、无付费 API、不上传任何图片',
    ],
    boundaries: [
      ...
      'ℹ️ 速度说明：常规图 200–600ms；4K+ 超大图也稳定不冻结（内部自动缩到最长边 1600 处理，最终仍按原尺寸输出）',
      'ℹ️ 纯前端本地处理：不存储、不上传用户任何图片数据；无模型下载，打开即用',
    ],
    boundaryCardStyle: 'amber',
    accept: 'image/*',
    multiple: true,
